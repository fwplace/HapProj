...
## This version add one parameter maf_sids, which calcuates MAFs of SNP/Haps in a subset of sampels,
## e.g., BCa cases only
gdsCoxFit <- function(set.idx, gdsfn, pheno, yy, xx, type, frq.min, frq.max, maf_sids, haps, saveGeno=FALSE){
  ...
  if(type=='snp'){
	...
	if(length(vidx)>0){
        ...
        res <- data.frame(vid=seqGetData(gds, 'variant.id')[vidx], 
                          ea=seqGetData(gds, '$alt')[vidx], nea=seqGetData(gds, '$ref')[vidx], 
                          eaf=eaf[vidx], fit)
  ...
  if(type=='hap'){
    ...
    if(length(hidx)>0){
       ...
       if(nrow(hinfo)==1){
              ...
              hmat <- do.call('cbind', lapply(hinfo$hap,function(x){as.numeric(h1==x)})) + 
                      do.call('cbind', lapply(hinfo$hap, function(x){as.numeric(h2==x)}))
       ...
       res <- data.frame(block=paste(chr[1], set.idx[1], set.idx[length(set.idx)], sep='_'), 
                         chr=chr[1], start=min(pos), end=max(pos), t0=length(haps), t1=nrow(hinfo), hinfo, fit)
