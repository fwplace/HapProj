...
gdsCoxFit <- function(set.idx, gdsfn, pheno, yy, xx, type, frq.min, frq.max, maf_sids, haps, test='ind', saveGeno=FALSE){
  ...
  if(type=='snp'){
	...
	if(length(vidx)>0){
        if(length(vidx)==1){
               ...
               geno <- geno[, vidx]
               if(test=='ind')  fit <- t(apply(geno, 2, coxFit, nz=1, pheno[, yy], pheno[, xx]))
               if(test=='join') fit <- coxFit(geno, nz=ncol(geno), pheno[, yy], pheno[, xx])
        }
        res <- data.frame(vid=seqGetData(gds, 'variant.id')[vidx], ea=seqGetData(gds, '$alt')[vidx], nea=seqGetData(gds, '$ref')[vidx], eaf=eaf[vidx], fit)
  ...
  if(type=='hap'){
    ...
    if(length(hidx)>0){
       ...
       hinfo$eaf <- as.numeric(sapply(as.vector(unlist(hinfo$hap)), function(x){sum(hh==x)/nn}))
       hinfo <- hinfo[order(-hinfo$eaf.case), ]
       hinfo <- data.frame(hid=paste('h',1:length(hidx),sep=''),hinfo)
       ...
       if(nrow(hinfo)==1){
              ...
              fit  <- t(data.frame(fit))
               ...
               if(test=='ind')  fit <- t(apply(hmat, 2, coxFit, nz=1, pheno[, yy], pheno[, xx]))
               if(test=='join') fit <- coxFit(hmat, nz=ncol(hmat), pheno[, yy], pheno[, xx])    
       }
       res <- data.frame(block=paste(chr[1], set.idx[1], set.idx[length(set.idx)], sep='_'), chr=chr[1], start=min(pos), end=max(pos), tot=nrow(hinfo), hinfo, fit)
